...
void Default_Invoise::setId()
{
	ifstream myfile;
	myfile.open("file_Id.txt");
	myfile >> this->Id;
	myfile.close();
	this->Id++;
	ofstream file;
	file.open("file_Id.txt", std::ios::trunc);
	file << this->Id;
	file.close();
}

void Default_Invoise::setSize()
{
...
void Default_Invoise::setPrice()
{
	price = box.getName() * 5 + 20 + this->weight * 20;
}

void Default_Invoise::setSender(const std::string& name)
{
...
void Default_Invoise::setReceiver(const std::string& name)
{
	for (size_t i = 0; i < SIZE; i++)
	{
		if (i < name.length())
...
void Default_Invoise::setAdress(const std::string& adress)
{
	for (size_t i = 0; i < SIZE; i++)
	{
		if (i < adress.length())
...
void Default_Invoise::setSity(const std::string& city)
{
	for (size_t i = 0; i < SIZE; i++)
	{
		if (i < city.length())
...
void Default_Invoise::PrintInvoise() const
{
...
size_t Default_Invoise::getId() const
{
	return this->Id;
}

size_t Default_Invoise::getPrice() 
{
	return this->price;
}

void Default_Invoise::setPay_()
{
	char s = 'y';
...
void Default_Invoise::closeInvoise()
{
	this->success = true;
}

std::string Default_Invoise::getName() const
{
	return string(this->receiver);
}

void Default_Invoise::editInvoise()
{
...
bool Default_Invoise::getSuccess() const
{
	return this->success;
}
